    ...
    <ant-form
      ...
      <ant-form-item ref="email" label="邮箱" name="email" class="custom-input-style">
        <div style="display: flex">
          <ant-input v-model:value="signUpFormState.email" style="flex: 1"/>
          <ant-button style="margin-left: 15px" type="primary"
                      :disabled="!signUpFormState.email">
            <span v-if="!isEnding">发送验证码</span>
            <end-time v-else @timeEnd="isEnding = false;" :endTime="endTime"></end-time>
          </ant-button>
        </div>
      </ant-form-item>
      <ant-form-item ref="code" label="用户名" name="验证码" class="custom-input-style">
        <ant-input autocomplete="off" v-model:value="signUpFormState.code"/>
      </ant-form-item>
