/**
 * @author songxiwen
 * @date  2022/1/17 23:34
 */

import KnowledgePictures from './knowledge-pictures.vue';
import KnowledgeArticleControlToolbar from './knowledge-article-control-toolbar.vue';
import KnowledgeRelationEdgesPanel from './knowledge-relation-edges-panel.vue';
import KnowledgeEditHeader from './knowledge-editor-header.vue';
import KnowledgeMentionedList from './knowledge-mentioned-list.vue';
import KnowledgeCustomFields from './knowledge-custom-fields.vue';
import KnowledgeSidebar from './knowledge-sidebar.vue';
import KnowledgeBaseMessageForm from './knowledge-base-message-form.vue';

export {
  KnowledgeArticleControlToolbar,
  KnowledgePictures,
  KnowledgeRelationEdgesPanel,
  KnowledgeEditHeader,
  KnowledgeMentionedList,
  KnowledgeCustomFields,
  KnowledgeSidebar,
  KnowledgeBaseMessageForm
};
