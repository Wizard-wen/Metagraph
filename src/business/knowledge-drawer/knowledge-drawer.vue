  ...
  <ant-drawer
    ...
    <div v-if="knowledge">
      <ant-descriptions title="基本信息" :column="1">
        ...
        <ant-descriptions-item label="类型">
          {{ knowledge.content.knowledgeBaseTypeName }}
        </ant-descriptions-item>
        <ant-descriptions-item label="作者">{{ knowledge.author.name }}</ant-descriptions-item>
        <ant-descriptions-item label="知识库">
          {{ knowledge.content.repositoryName }}
        </ant-descriptions-item>
      </ant-descriptions>
      <ant-descriptions title="热度" :column="1">
        ...
        <ant-descriptions-item label="标签">
          <ant-tag v-for="(item, index) in knowledge.tag" :key="index">{{ item.name }}</ant-tag>
        </ant-descriptions-item>
      </ant-descriptions>
      <h3>概述</h3>
